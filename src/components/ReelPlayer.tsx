...
export default function ReelPlayer({
    ...
    useEffect(() => {
        ...
        // HORIZONTAL NAVIGATION (alternate versions or related content)
        if (reel.alternateVersions && reel.alternateVersions.length > 0) {
            ...
            // Preload ALL alternates for instant navigation
            reel.alternateVersions.forEach(versionId => {
                if (versionId !== reel.id) {
                    const alternateReel = reels.find(r => r.id === versionId);
                    if (alternateReel && !preloadRefs.current[alternateReel.id]) {
                        const preloadVideo = document.createElement('video');
                        preloadVideo.src = alternateReel.videoUrl;
                        preloadVideo.muted = true;
                        preloadVideo.preload = 'auto';
                        preloadVideo.load();
                        preloadRefs.current[alternateReel.id] = preloadVideo;
                    }
                }
            });
        } else if (reel.tags && reel.tags.length > 0) {
            // Find related reels by tags if no alternate versions
            const relatedReels = reels.filter(
                r => r.id !== reel.id &&
                    r.tags.some(tag => reel.tags.includes(tag))
            ).slice(0, 4); // Limit to 4 related reels for performance
            ...
            if (relatedReels.length > 0) {
                ...
                // Preload related reels (but with lower priority)
                relatedReels.forEach(relatedReel => {
                    if (!preloadRefs.current[relatedReel.id]) {
                        // Use setTimeout to delay loading of related reels
                        // (prioritize series episodes/alternates)
                        setTimeout(() => {
                            if (!preloadRefs.current[relatedReel.id]) {
                                const preloadVideo = document.createElement('video');
                                preloadVideo.src = relatedReel.videoUrl;
                                preloadVideo.muted = true;
                                preloadVideo.preload = 'auto';
                                preloadVideo.load();
                                preloadRefs.current[relatedReel.id] = preloadVideo;
                            }
                        }, 500);
                    }
                });
    ...
    }, [isPlaying, isMuted, showWhatIf, showInfo, showKeyboardHelp, isTransitioning, currentReel]);
    ...
    const canNavigateUp = Boolean(currentReel.seriesId && currentReel.episodeNumber);
    const canNavigateDown = Boolean(currentReel.seriesId && currentReel.episodeNumber);
    const canNavigateHorizontal = Boolean(
            ...
            <div className="relative w-full h-full bg-background">
                ...
                >
                    ...
                    <div className="absolute bottom-20 md:bottom-8 left-4 right-20">
                        ...
                        <div className="flex flex-wrap gap-2 mb-2">
                            {currentReel.tags.slice(0, 3).map((tag, index) => (
                                <span key={index} className="text-xs bg-white/10 backdrop-blur-sm px-2 py-0.5 rounded-full">
                                    {tag}
                                </span>
                            ))}
                        </div>
                    ...
                    <div className="absolute right-4 bottom-24 md:bottom-28 flex flex-col items-center space-y-4">
                        <motion.button
                            className="w-10 h-10 flex items-center justify-center bg-primary/20 backdrop-blur-sm rounded-full"
                            whileHover={{ scale: 1.1 }}
                            whileTap={{ scale: 0.9 }}
                        >
                            <FaHeart className="text-xl text-white" />
                            <span className="absolute -bottom-5 text-xs">{currentReel.likes}</span>
                        </motion.button>
                        ...
                        {currentReel.whatIfScenarios && currentReel.whatIfScenarios.length > 0 && (
                            <motion.button
                                className={`w-10 h-10 flex items-center justify-center ${showWhatIf ? 'bg-primary' : 'bg-primary/20'} backdrop-blur-sm rounded-full`}
                                whileHover={{ scale: 1.1 }}
                                whileTap={{ scale: 0.9 }}
                    ...
                    {/* Desktop Arrow Controls with Enhanced UI */}
                    {showArrows && (
                        <>
                            {/* Next Episode (Up) Arrow */}
                            {nextReels.up && (
                                <motion.button
                                    className="absolute top-20 left-1/2 transform -translate-x-1/2 w-14 h-14 rounded-full bg-gradient-to-b from-primary/30 to-black/60 backdrop-blur-md flex items-center justify-center border border-primary/30 hidden md:flex z-20"
                ...
                {/* Mobile Navigation Hints - Always visible but subtle */}
                {!isDesktop && (
                    <div className="absolute inset-0 pointer-events-none">
                        {/* Up: Next in Series */}
                        {canNavigateUp && (
                            <div className="absolute top-16 left-1/2 transform -translate-x-1/2 w-10 h-10 flex items-center justify-center">
                                <motion.div
                                    className="w-8 h-8 rounded-full bg-white/5 backdrop-blur-sm flex items-center justify-center"
                                    animate={{ y: [0, -5, 0] }}
                                    transition={{ duration: 2, repeat: Infinity }}
                                >
                                    <FaArrowUp className="text-sm text-white/50" />
                                </motion.div>
                            </div>
                        )}
                        ...
                        {canNavigateDown && (
                            <div className="absolute bottom-24 left-1/2 transform -translate-x-1/2 w-10 h-10 flex items-center justify-center">
                                <motion.div
                                    className="w-8 h-8 rounded-full bg-white/5 backdrop-blur-sm flex items-center justify-center"
                                    animate={{ y: [0, 5, 0] }}
                                    transition={{ duration: 2, repeat: Infinity }}
                                >
                                    <FaArrowDown className="text-sm text-white/50" />
                                </motion.div>
                            </div>
                        )}

                        {/* Left/Right: Alternate Versions/Related */}
                        {canNavigateHorizontal && (
                            <>
                                <div className="absolute left-4 top-1/2 transform -translate-y-1/2 w-10 h-10 flex items-center justify-center">
                                    <motion.div
                                        className="w-8 h-8 rounded-full bg-white/5 backdrop-blur-sm flex items-center justify-center"
                                        animate={{ x: [0, -5, 0] }}
                                        transition={{ duration: 2, repeat: Infinity }}
                                    >
                                        <FaArrowLeft className="text-sm text-white/50" />
                                    </motion.div>
                                </div>

                                <div className="absolute right-4 top-1/2 transform -translate-y-1/2 w-10 h-10 flex items-center justify-center">
                                    <motion.div
                                        className="w-8 h-8 rounded-full bg-white/5 backdrop-blur-sm flex items-center justify-center"
                                        animate={{ x: [0, 5, 0] }}
                                        transition={{ duration: 2, repeat: Infinity }}
                                    >
                                        <FaArrowRight className="text-sm text-white/50" />
                                    </motion.div>
                                </div>
                            </>
                        )}
                    ...
                    {showWhatIf && currentReel.whatIfScenarios && currentReel.whatIfScenarios.length > 0 && (
                        ...
                        >
                            <div className="flex justify-between items-center mb-6">
                                <div className="flex items-center">
                                    <FaLightbulb className="text-primary-light text-xl mr-2" />
                                    <h3 className="text-2xl font-bold">What If Scenarios</h3>
                                </div>
                            ...
                            <div className="overflow-y-auto max-h-[calc(100vh-150px)]">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {currentReel.whatIfScenarios.map((scenario: WhatIfScenario) => (
                                        <motion.div
                                            key={scenario.id}
                                            className="bg-white/5 backdrop-blur-sm rounded-xl p-4 border border-primary/20"
                                            whileHover={{ scale: 1.02, backgroundColor: "rgba(255,255,255,0.1)" }}
                                        >
                                            <h3 className="text-lg font-bold text-primary-light mb-2">{scenario.title}</h3>
                                            <p className="text-sm mb-4">{scenario.description}</p>
                                            <motion.button
                                                className="px-4 py-2 bg-primary/30 hover:bg-primary/50 rounded-full text-sm flex items-center"
                                                whileHover={{ scale: 1.05 }}
                                                whileTap={{ scale: 0.95 }}
                                            >
                                                <FaPlay className="mr-2 text-xs" /> Explore this path
                                            </motion.button>
                                        </motion.div>
                                    ))}
                                </div>
                                ...
                                <div className="mb-6">
                                    ...
                                    <div className="flex flex-wrap gap-2 mb-6">
                                        {currentReel.tags.map((tag, index) => (
                                            <span key={index} className="text-sm bg-white/10 backdrop-blur-sm px-3 py-1 rounded-full">
                                                {tag}
                                            </span>
                                        ))}
                                    </div>
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center">
                                                ...
                                                <span>Next alternate version</span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center">
                                                ...
                                                <span>Previous alternate version</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
